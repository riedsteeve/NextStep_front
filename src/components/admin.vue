...
<template>
  <main class="min-h-screen bg-gray-200 px-4 py-6 text-slate-800 sm:px-6 md:p-8">
    <section class="mx-auto flex w-full max-w-7xl flex-col gap-6">
      <div class="rounded-2xl bg-white p-5 w-full shadow-sm border border-slate-200 sm:p-6">
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <button
            type="button"
            @click="handleLogout"
            class="w-full min-h-12 rounded-xl bg-purple-700 px-4 py-3 font-bold text-white shadow-md transition hover:bg-purple-800 active:scale-95 md:w-auto"
          >
            Déconnexion
          </button>
        </div>
      ...
      <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p class="text-sm font-bold uppercase tracking-wide text-slate-500">Utilisateurs</p>
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
        <div class="min-h-44 rounded-xl bg-purple-600 p-5 text-white shadow-sm"
          v-for="Compte in utilisateurs"
          :key="Compte.id"
        >
          <div class="flex h-full flex-col gap-3">
            <div class="min-w-0">
              <p class="text-xs font-bold uppercase tracking-wide text-purple-100">Utilisateur</p>
